...
exports.getAllStudents = async (req, res, next) => {
    try {
        ...
        res.status(400).json({
            message: `Error in getting all students", ${err.message}`,
        });
    }
};
...
exports.getAllFaculty = async (req, res, next) => {
    try {
        ...
        res.status(400).json({
            message: `Error in getting all faculties", ${err.message}`,
        });
    }
};
...
exports.getAnnouncements = async (req, res, next) => {
    try {
        const results = await Announcement.find({}, { author_id: 0, __v: 0 })
            .sort({ _id: -1 })
            .exec();
...
exports.deleteAnnouncement = async (req, res, next) => {
    ...
    try {
        ...
        const announcementExists = await Announcement.findOne({ _id: id });
        ...
        await announcementExists.deleteOne();

        res.status(200).json({ message: "Announcemnt deleted." });
    } catch (err) {
        console.log(err);
        return res.status(500).json({ message: "Something went wrong." });
    }
...
exports.getEvents = async (req, res, next) => {
    try {
        const results = await Event.find({}, { author_id: 0, __v: 0 })
            .sort({ _id: -1 })
            .exec();
        res.status(200).json({ result: results });
    } catch (err) {
...
exports.deleteEvent = async (req, res, next) => {
    ...
    try {
        ...
        const eventExists = await Event.findOne({ _id: id });
