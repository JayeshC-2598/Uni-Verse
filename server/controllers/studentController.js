    ...
    try {
        ...
        res.status(200).json({
            result: attendance.map((att) => {
                ...
                let res = {};
                res.attendance = (
                    (att.totalLectures / att.subject.totalLectures) *
                    100
                ).toFixed(2);
                ...
                res.attendedLectures = att.totalLectures;
                res.totalLectures = att.subject.totalLectures;
                return res;
            }),
        });
    } catch (err) {
...
exports.getAllSubjects = async (req, res, next) => {
    try {
        ...
        return res
            .status(400)
            .json({ "Error in fetching subjects": err.message });
    }
};
    ...
    try {
        ...
        for (var i = 0; i < filteredListPro.length; i++) {
            for (var j = 0; j < filteredListTemp.length; j++) {
                if (
                    filteredListPro[i].senderName ===
                    filteredListTemp[j].receiverName
                ) {
